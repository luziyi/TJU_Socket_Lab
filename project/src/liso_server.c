...
int main(int argc, char *argv[])
{
    ...
    {
        log("liso.log", "Failed creating socket.", LOG_LEVEL_ERROR, NULL);
    ...
    {
        close_socket(sock);
        log("liso.log", "Failed binding socket.", LOG_LEVEL_ERROR, NULL);
    ...
    {
        close_socket(sock);
        log("liso.log", "Error listening on socket.", LOG_LEVEL_ERROR, NULL);
    ...
    while (1)
    {
        ...
        {
            close(sock);
            log("liso.log", "Error accepting connection.", LOG_LEVEL_ERROR, NULL);
        ...
        while ((readret = recv(client_sock, buf, BUF_SIZE, 0)) >= 1)
        {
            ...
            {
                log("liso.log", "Message buffer overflow.", LOG_LEVEL_ERROR, client_ip);
            ...
            while (end_of_message != NULL)
            {
                ...
                if (log_message != NULL)
                    log("liso.log", log_message, LOG_LEVEL_INFO, client_ip);
